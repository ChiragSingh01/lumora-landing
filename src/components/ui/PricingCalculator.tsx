...
export default function PricingCalculator() {
  ...
  const total =
    users * plans[plan] +
    selectedAddons.reduce((acc, addon) => acc + addons[addon as keyof typeof addons], 0)
  ...
  return (
    <Card className="max-w-xl mx-auto p-8 glass-effect shadow-xl rounded-2xl border border-gray-200 dark:border-gray-700 mt-20">
      ...
      <div className="mb-6">
        <label className="block font-semibold mb-2 text-gray-800 dark:text-gray-200 flex items-center gap-2">
          <User className="text-green-500" /> Number of Users
        </label>
        <input
          type="number"
          min={1}
      ...
      <div className="mb-6">
        <label className="block font-semibold mb-2 text-gray-800 dark:text-gray-200 flex items-center gap-2">
          <LineChart className="text-purple-500" /> Select Plan
        </label>
        <select
          value={plan}
      ...
      <div className="mb-6">
        ...
        <div className="space-y-3">
          {Object.entries(addons).map(([name, price]) => (
            <label key={name} className="flex items-center space-x-3 text-gray-800 dark:text-gray-200">
              <input
                type="checkbox"
